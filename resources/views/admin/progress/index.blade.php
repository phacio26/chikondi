                ...
                <tbody>
                    @foreach($progress as $item)
                        <tr class="border-b border-accent/5 hover:bg-surface transition-all">
                            <td class="px-6 py-5">
                                @if($item->image)
                                    <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->title }}" class="w-12 h-12 rounded-xl object-cover">
                                @else
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-4">
                                    ...
                                    <form method="POST" action="{{ route('admin.progress.destroy', $item) }}" class="inline" id="delete-form-{{ $item->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete(null, 'Delete this progress update? This action cannot be undone.', document.getElementById('delete-form-{{ $item->id }}'))" class="text-[10px] font-black uppercase tracking-widest text-brand hover:text-accent transition-all">
                                            Delete
                                        </button>
                                    </form>
                                </div>
