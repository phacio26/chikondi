...
<div class="bg-white rounded-2xl border border-accent/5 p-6 md:p-10 max-w-3xl">
    ...
    <form method="POST" action="{{ route('admin.team.store') }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="role" value="{{ old('role') }}" placeholder="e.g. Technical Services Manager"
                   class="w-full px-4 py-3 bg-surface border border-accent/20 rounded-xl focus:border-brand outline-none text-sm font-semibold text-accent">
            @error('role')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            <label class="block text-[11px] font-black uppercase tracking-widest text-accent mb-2">Photo</label>
        ...
        <div>
            ...
            <textarea name="bio" rows="8" placeholder="Qualifications, background, certifications..."
                      class="w-full px-4 py-3 bg-surface border border-accent/20 rounded-xl focus:border-brand outline-none text-sm font-semibold text-accent">{{ old('bio') }}</textarea>
            @error('bio')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
